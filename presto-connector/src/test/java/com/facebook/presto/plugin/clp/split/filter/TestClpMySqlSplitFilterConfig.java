...
public class TestClpMySqlSplitFilterConfig {
    ...
    private <T> void testRange(
            T lowerBound,
            T upperBound,
            ClpMySqlSplitFilterProvider filterProvider
    ) {
        String splitFilterSql1 = format(
                "(\"msg.timestamp\" > %s AND \"msg.timestamp\" < %s)",
                lowerBound,
                upperBound
        );
        String remappedSql1 = filterProvider.remapSplitFilterPushDownExpression(
                "clp.default.table_1",
                splitFilterSql1
        );
        assertEquals(
                remappedSql1,
                format("(end_timestamp > %s AND begin_timestamp < %s)", lowerBound, upperBound)
        );

        String splitFilterSql2 = format(
                "(\"msg.timestamp\" >= %s AND \"msg.timestamp\" <= %s)",
                lowerBound,
                upperBound
        );
        String remappedSql2 = filterProvider.remapSplitFilterPushDownExpression(
                "clp.default.table_1",
                splitFilterSql2
        );
        assertEquals(
        ...
        String splitFilterSql3 = format(
                "(\"msg.timestamp\" > %s AND \"msg.timestamp\" <= %s)",
                lowerBound,
                upperBound
        );
        String remappedSql3 = filterProvider.remapSplitFilterPushDownExpression(
                "clp.default.table_1",
                splitFilterSql3
        );
        assertEquals(
        ...
        String splitFilterSql4 = format(
                "(\"msg.timestamp\" >= %s AND \"msg.timestamp\" < %s)",
                lowerBound,
                upperBound
        );
        String remappedSql4 = filterProvider.remapSplitFilterPushDownExpression(
                "clp.default.table_1",
                splitFilterSql4
        );
        assertEquals(
        ...
        String remappedSql5 = filterProvider.remapSplitFilterPushDownExpression(
                "clp.default.table_1",
                splitFilterSql5
        );
        assertEquals(
                remappedSql5,
                format("((begin_timestamp <= %s AND end_timestamp >= %s))", lowerBound, lowerBound)
        );
    }
}
