...
public class TestClpQueryConfig extends TestClpQueryBase {
    ...
    private Session transactionSession() {
        ...
        return testSessionBuilder().setCatalog("clp").setSchema("default").setTransactionId(
                transactionId
        ).build();
    }
    ...
    @Test(
            ...
            expectedExceptionsMessageRegExp = ".*Unsupported CLP_QUERY_CONFIG key.*"
    )
    public void testQueryConfigUnsupportedKey() {
        Session session = transactionSession();
        optimize(
                session,
                "SELECT * FROM test WHERE CLP_QUERY_CONFIG('bogus_key', 'true') AND isHoliday = true"
        );
    }

    @Test(
            expectedExceptions = PrestoException.class,
            expectedExceptionsMessageRegExp = ".*Invalid value.*"
    )
    public void testQueryConfigInvalidValue() {
        Session session = transactionSession();
        optimize(
                session,
                "SELECT * FROM test WHERE CLP_QUERY_CONFIG('case_insensitive', 'yes') AND isHoliday = true"
        );
    }

    @Test(
            expectedExceptions = PrestoException.class,
            expectedExceptionsMessageRegExp = ".*varchar literal.*"
    )
    public void testQueryConfigNonLiteralArguments() {
        Session session = transactionSession();
        optimize(
                session,
                "SELECT * FROM test WHERE CLP_QUERY_CONFIG(city.Name, 'true') AND isHoliday = true"
        );
    }
    ...
    @Test(
            expectedExceptions = PrestoException.class,
            expectedExceptionsMessageRegExp = ".*Expected a boolean literal.*"
    )
    ...
    public void testQueryConfigUnderOrIsNotExtracted() {
        ...
        assertOptimizedPlan(
                ...
                anyTree(
                        filter(
                                expression(
                                        "CLP_QUERY_CONFIG('case_insensitive', true) OR isHoliday = true"
                                ),
                                ClpTableScanMatcher.clpTableScanPattern(
                                        new ClpTableLayoutHandle(
                                                table,
