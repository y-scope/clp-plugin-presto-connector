...
public class TestClpSessionProperties {
    ...
    public void testCaseInsensitiveDefaultsToConfigValue() {
        PropertyMetadata<?> property = getCaseInsensitiveProperty(
                new ClpConfig().setCaseInsensitive(true)
        );
    ...
    private static PropertyMetadata<?> getCaseInsensitiveProperty(ClpConfig config) {
        ...
        PropertyMetadata<?> property = sessionProperties.getSessionProperties().stream().filter(
                propertyMetadata -> CASE_INSENSITIVE.equals(propertyMetadata.getName())
        ).findFirst().orElseThrow(
                () -> new AssertionError("case_insensitive session property not registered")
        );
