...
public class ClpFilterToKqlConverter implements RowExpressionVisitor<ClpExpression, Void> {
    ...
    private ClpExpression handleNot(CallExpression node) {
        ...
        return expression.getMetadataExpression().isPresent() ? new ClpExpression(
    ...
    private ClpExpression handleAnd(SpecialFormExpression node) {
        ...
        if (pushDownExpressions.isEmpty()) { return new ClpExpression(node); }
    ...
    private ClpExpression handleOr(SpecialFormExpression node) {
        ...
        for (RowExpression argument : node.getArguments()) {
            ...
            if (
                expression.getRemainingExpression().isPresent() || false == expression
