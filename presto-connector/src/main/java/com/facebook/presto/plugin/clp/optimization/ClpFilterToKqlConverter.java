 ...
 * </ul>
 */
public class ClpFilterToKqlConverter implements RowExpressionVisitor<ClpExpression, Void> {
    private static final Set<OperatorType> LOGICAL_BINARY_OPS_FILTER = ImmutableSet.of(
            EQUAL,
            NOT_EQUAL,
            LESS_THAN,
            LESS_THAN_OR_EQUAL,
    ...
    public ClpFilterToKqlConverter(
        ...
        this.standardFunctionResolution = requireNonNull(
                standardFunctionResolution,
                "standardFunctionResolution is null"
        );
        this.functionMetadataManager = requireNonNull(
                functionMetadataManager,
                "function metadata manager is null"
        );
        this.assignments = requireNonNull(assignments, "assignments is null");
        this.metadataFilterColumns = requireNonNull(
                metadataFilterColumns,
                "metadataFilterColumns is null"
        );
    }
    ...
    public ClpExpression visitCall(CallExpression node, Void context) {
        ...
        if (standardFunctionResolution.isNotFunction(functionHandle)) { return handleNot(node); }

        if (standardFunctionResolution.isLikeFunction(functionHandle)) { return handleLike(node); }

        FunctionMetadata functionMetadata = functionMetadataManager.getFunctionMetadata(
        ...
        if (operatorTypeOptional.isPresent()) {
            ...
            if (BETWEEN == operatorType) { return handleBetween(node); }
        }

        String functionName = functionMetadata.getName().getObjectName().toUpperCase();
        if (functionName.startsWith("CLP_WILDCARD_")) { return new ClpExpression("*"); }
    ...
    private ClpExpression handleBetween(CallExpression node) {
        ...
        if (
            !isClpCompatibleNumericType(first.getType()) || !isClpCompatibleNumericType(
                    second.getType()
        ...
        if (
            !variableOpt.isPresent() || !(second instanceof ConstantExpression)
        ...
        String kql = String.format(
                "%s >= %s AND %s <= %s",
                variable,
                formatLiteral(second.getType(), lowerBound),
                variable,
                formatLiteral(third.getType(), upperBound)
        );
        String metadataSqlQuery = metadataFilterColumns.contains(variable) ? String.format(
                "\"%s\" >= %s AND \"%s\" <= %s",
                variable,
                lowerBound,
                variable,
    ...
    private ClpExpression handleNot(CallExpression node) {
        ...
        if (
            expression.getRemainingExpression().isPresent() || !expression.getPushDownExpression()
                    .isPresent()
        ) {
            return new ClpExpression(node);
        }
        ...
        if (expression.getMetadataSqlQuery().isPresent()) {
            return new ClpExpression(
                    notPushDownExpression,
                    "NOT " + expression.getMetadataSqlQuery()
            );
        } else {
    ...
    private ClpExpression handleLike(CallExpression node) {
        ...
        if (!variable.getPushDownExpression().isPresent()) { return new ClpExpression(node); }
    ...
    private ClpExpression handleLogicalBinary(OperatorType operator, CallExpression node) {
        ...
        Optional<ClpExpression> maybeLeftSubstring = tryInterpretSubstringEquality(
                operator,
                left,
                right
        );
        if (maybeLeftSubstring.isPresent()) { return maybeLeftSubstring.get(); }

        Optional<ClpExpression> maybeRightSubstring = tryInterpretSubstringEquality(
                operator,
                right,
                left
        );
        if (maybeRightSubstring.isPresent()) { return maybeRightSubstring.get(); }
    ...
    private ClpExpression buildClpExpression(
        ...
        if (operator.equals(EQUAL)) {
            if (literalType instanceof VarcharType) {
                ...
                return new ClpExpression(
                        format("%s: %s", variableName, formatLiteral(literalType, literalString)),
                        metadataSqlQuery
                );
            }
            ...
            if (literalType instanceof VarcharType) {
                ...
                return new ClpExpression(
                        format(
                                "NOT %s: %s",
                                variableName,
                                formatLiteral(literalType, literalString)
                        ),
                        metadataSqlQuery
                );
            }
            ...
            if (
                LOGICAL_BINARY_OPS_FILTER.contains(operator)
                        && !(literalType instanceof VarcharType)
            ) {
                if (metadataFilterColumns.contains(variableName)) {
                    metadataSqlQuery = format(
                            "\"%s\" %s %s",
                            variableName,
    ...
    private Optional<ClpExpression> tryInterpretSubstringEquality(
        ...
        if (!operator.equals(EQUAL) && !operator.equals(NOT_EQUAL)) { return Optional.empty(); }

        if (
            !(possibleSubstring instanceof CallExpression)
                    || !(possibleLiteral instanceof ConstantExpression)
        ) {
            return Optional.empty();
        }

        Optional<SubstrInfo> maybeSubstringCall = parseSubstringCall(
                (CallExpression)possibleSubstring
        );
        if (!maybeSubstringCall.isPresent()) { return Optional.empty(); }

        String targetString = getLiteralString((ConstantExpression)possibleLiteral);
        return interpretSubstringEquality(
                maybeSubstringCall.get(),
                targetString,
    ...
    private Optional<SubstrInfo> parseSubstringCall(CallExpression callExpression) {
        ...
        if (!functionName.equals("substr")) { return Optional.empty(); }

        int argCount = callExpression.getArguments().size();
        if (argCount < 2 || argCount > 3) { return Optional.empty(); }

        ClpExpression variable = callExpression.getArguments().get(0).accept(this, null);
        if (!variable.getPushDownExpression().isPresent()) { return Optional.empty(); }
     ...
     */
    private Optional<ClpExpression> interpretSubstringEquality(
            SubstrInfo info,
            String targetString,
            boolean isEqual
    ) {
     ...
     */
    private Optional<Integer> parseLengthLiteral(
        ...
        if (lengthExpression instanceof ConstantExpression) {
            ...
            try {
                ...
                if (parsed == targetString.length()) { return Optional.of(parsed); }
            } catch (NumberFormatException ignored) {}
        }
        return Optional.empty();
    }
        ...
        if (!hasPushDownExpression) {
            ...
            if (remainingExpressions.size() == 1) {
                return new ClpExpression(
                        ...
                        hasMetadataSql ? metadataQueryBuilder.substring(
                                0,
                                metadataQueryBuilder.length() - 5
                        ) + ")" : null,
    ...
    private ClpExpression handleOr(SpecialFormExpression node) {
        ...
        for (RowExpression argument : node.getArguments()) {
            ClpExpression expression = argument.accept(this, null);
            // Note: It is possible in the future that an expression cannot be pushed down as a KQL
            // query, but can be
            // pushed down as a metadata SQL query.
            if (
                expression.getRemainingExpression().isPresent() || !expression
                        .getPushDownExpression().isPresent()
            ) {
                allPushedDown = false;
                continue;
            }
        ...
        if (allPushedDown) {
            ...
            return new ClpExpression(
                    ...
                    hasAllMetadataSql ? metadataQueryBuilder.substring(
                            0,
                            metadataQueryBuilder.length() - 4
                    ) + ")" : null
            );
        }
        return new ClpExpression(node);
    ...
    private ClpExpression handleIn(SpecialFormExpression node) {
        ...
        for (RowExpression argument : node.getArguments().subList(1, node.getArguments().size())) {
            if (!(argument instanceof ConstantExpression)) { return new ClpExpression(node); }
            ConstantExpression literal = (ConstantExpression)argument;
    ...
    private ClpExpression handleIsNull(SpecialFormExpression node) {
        ...
        if (!expression.getPushDownExpression().isPresent()) { return new ClpExpression(node); }
    ...
    private ClpExpression handleDereference(RowExpression expression) {
        ...
        if (arguments.size() != 2) {
            throw new PrestoException(
                    CLP_PUSHDOWN_UNSUPPORTED_EXPRESSION,
                    "DEREFERENCE expects 2 arguments"
            );
        }
        ...
        if (!(index instanceof ConstantExpression)) {
            throw new PrestoException(
                    CLP_PUSHDOWN_UNSUPPORTED_EXPRESSION,
                    "DEREFERENCE index must be a constant"
            );
        }
        ...
        if (!(value instanceof Long)) {
            throw new PrestoException(
                    CLP_PUSHDOWN_UNSUPPORTED_EXPRESSION,
                    "DEREFERENCE index constant is not a long"
            );
        }
        ...
        if (!(baseType instanceof RowType)) {
            throw new PrestoException(
                    CLP_PUSHDOWN_UNSUPPORTED_EXPRESSION,
                    "DEREFERENCE base is not a RowType: " + baseType
            );
        }
    ...
    public static boolean isClpCompatibleNumericType(Type type) {
        return type.equals(BIGINT) || type.equals(INTEGER) || type.equals(SMALLINT) || type.equals(
                TINYINT
        ) || type.equals(DOUBLE) || type.equals(REAL) || type.equals(TIMESTAMP) || type.equals(
