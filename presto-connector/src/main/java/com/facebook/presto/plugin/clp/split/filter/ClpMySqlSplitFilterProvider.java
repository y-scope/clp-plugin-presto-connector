 ...
 */
public class ClpMySqlSplitFilterProvider extends ClpSplitFilterProvider {
    @Inject
    ...
    public String remapSplitFilterPushDownExpression(String scope, String pushDownExpression) {
        ...
        Map<String, ClpMySqlCustomSplitFilterOptions.RangeMapping> mappings = new HashMap<>(
                getAllMappingsFromFilters(filterMap.get(splitScope[0]))
        );

        if (1 < splitScope.length) {
            mappings.putAll(
                    getAllMappingsFromFilters(filterMap.get(splitScope[0] + "." + splitScope[1]))
            );
        }
        ...
        for (
            Map.Entry<String, ClpMySqlCustomSplitFilterOptions.RangeMapping> entry : mappings
                    .entrySet()
        ) {
    ...
    private Map<String, ClpMySqlCustomSplitFilterOptions.RangeMapping> getAllMappingsFromFilters(
            List<ClpSplitFilterConfig> filters
    ) {
        return null != filters ? filters.stream().filter(
                filter -> filter.customOptions instanceof ClpMySqlCustomSplitFilterOptions
                        && ((ClpMySqlCustomSplitFilterOptions)filter.customOptions).rangeMapping
                                != null
        ).collect(
                toImmutableMap(
    ...
    protected static class ClpMySqlCustomSplitFilterOptions implements CustomSplitFilterOptions {
        ...
        public static class RangeMapping {
            ...
            public boolean equals(Object o) {
                ...
                if (!(o instanceof RangeMapping)) { return false; }
                RangeMapping that = (RangeMapping)o;
                return Objects.equals(lowerBound, that.lowerBound) && Objects.equals(
                        upperBound,
                        that.upperBound
                );
            }
