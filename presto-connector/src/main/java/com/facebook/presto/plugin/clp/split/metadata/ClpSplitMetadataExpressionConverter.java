 ...
 */
public class ClpSplitMetadataExpressionConverter implements RowExpressionVisitor<String, Void> {
    ...
    public String visitCall(CallExpression node, Void context) {
        ...
        if (false == operator.isPresent()) { throw unsupported(node); }

        if (OperatorType.NEGATION == operator.get()) {
            return "-" + node.getArguments().get(0).accept(this, null);
        }
        if (
            false == operator.get().isComparisonOperator() || OperatorType.IS_DISTINCT_FROM
                    == operator.get()
        ) {
        ...
        if (false == (left instanceof VariableReferenceExpression)) { throw unsupported(node); }
    ...
    private String againstBounds(
        ...
        switch (operator) {
            ...
            case GREATER_THAN_OR_EQUAL:
                return bounds.getUpper().map(
                        upper -> format("%s %s %s", upper, operator.getOperator(), literal)
                ).orElseThrow(() -> unsupported(node));
            case LESS_THAN:
            case LESS_THAN_OR_EQUAL:
                return bounds.getLower().map(
                        lower -> format("%s %s %s", lower, operator.getOperator(), literal)
                ).orElseThrow(() -> unsupported(node));
            case EQUAL:
                ...
                return bounds.getLower().map(lower -> format("%s <= %s", lower, literal)).orElseGet(
                        () -> format("%s >= %s", bounds.getUpper().get(), literal)
                );
            default:
                throw unsupported(node);
        }
    ...
    public String visitSpecialForm(SpecialFormExpression node, Void context) {
        switch (node.getForm()) {
            case AND:
            case OR:
                return node.getArguments().stream().map(
