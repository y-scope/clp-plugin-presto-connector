...
public class ClpColumnHandleCodec implements ConnectorCodec<ColumnHandle> {
        ...
        try {
            if (!(handle instanceof ClpColumnHandle)) {
                throw new IllegalArgumentException(
                        "Expected ClpColumnHandle but got: " + (handle == null ? "null"
        ...
        try {
            ...
            if (in.available() > 0) {
                throw new IOException(
                        "Unexpected trailing bytes in ClpColumnHandle deserialization"
                );
            }
