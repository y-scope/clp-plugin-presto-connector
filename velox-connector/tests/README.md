...
Unit tests for the parts of the connector that depend on nothing but the standard library.
...
A header is testable here if it includes nothing but the standard library. In practice that means
no `VELOX_*` macros, since each one constructs a Velox exception type, and no `config::ConfigBase`.
