...
if [[ -z "${version}" ]]; then
    version=$(sed -n 's|.*<version>\([^<]*\)</version>.*|\1|p' \
        "${src}/presto-connector/pom.xml" | head -1)
fi
...
echo "==> Building velox-connector .so..."
task -d "${src}/velox-connector" velox-connector:build
so_file="${src}/velox-connector/build/velox-connector/libclp-plugin-velox-connector.so"
...
presto_src="${src}/velox-connector/build/velox-connector/_deps/presto_native_execution-src"
...
bundle_velox_shared_libraries() {
    ...
    # Bundled libs: overwrite build-time RUNPATH with $ORIGIN.
    for lib in "${bundled_dir}"/*.so*; do
        [[ -f "${lib}" ]] || continue
        patchelf --set-rpath '$ORIGIN' "${lib}"
    done
    # Main .so: PREPEND $ORIGIN/lib — keep any existing plugin-loader paths.
    local old new='$ORIGIN/lib'
...
# Strip; leave debug-info to a future -dbgsym split.
strip --strip-unneeded "${velox_so_install}/libclp-plugin-velox-connector.so"
...
build_tarball() {
    ...
    local plugin_root
    plugin_root="$(dirname "${PRESTO_JAR_DIR}")"

    rm -rf "${staging}"
    mkdir -p "${staging}/${tar_dirname}"
    cp -a "${payload}${plugin_root}/." "${staging}/${tar_dirname}/"
